...
case class Game(user1: User, user2: User, completed: Boolean, lastUpdate: LocalDateTime,
                categories: List[Category], id: Option[UUID] = None) {
...
object Game {
  ...
  implicit object GameWriter extends BSONDocumentWriter[Game]:
    ...
    yield BSONDocument("_id" -> id, "user1" -> user1, "user2" -> user2, "completed" -> completed, "lastUpdate" -> lastUpdate,
      "category1" -> game.getCategories.head.getID,  "category2" -> game.getCategories(1).getID,  "category3" -> game.getCategories(2).getID)
}
