...
class RoundRepositoryTests extends AsyncFlatSpec with should.Matchers:
  ...
  val round: Round = rounds.filter(_.relatedGameID == game.id).head
  ...
  "A round" should "be read from the database by its related game's ID" in {
    roundRepository.readOne(BSONDocument("relatedGameID" -> game.id)).map(_.isDefined shouldEqual true)
  }

  "Rounds of a game" should "be read from the database by the belonging game" in {
    allRounds
      .map(
        _.map(_ should contain theSameElementsAs rounds.filter(_.relatedGameID == game.id))
